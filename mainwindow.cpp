...
void MainWindow::animacijaParametriButtonAktivni(bool param_aktivnosti)
{
    ...
    ui->rButton->setEnabled(!param_aktivnosti);
    ui->pbRect->setEnabled(!param_aktivnosti);
}
...
{
    if (ui->poluprecnik->text() != "" ){
        int poluprecnik = ui->poluprecnik->text().toInt();

        if (poluprecnik <= POLUPRECNIK){
            _poluprecnik = poluprecnik;
        }else{
            _poluprecnik = POLUPRECNIK;
        }
...
void MainWindow::napraviNoviAlgoritam()
{
    ...
    case TipAlgoritma::DEMO_ISCRTAVANJA:
        _pAlgoritamBaza = new DemoIscrtavanja(_pOblastCrtanja, _duzinaPauze, _naivni,
                                              _imeDatoteke, _brojSlucajnihObjekata, _poluprecnik);
        break;
    case TipAlgoritma::BRISUCA_PRAVA:
    ...
    case TipAlgoritma::TRIANGULACIJA:
        _pAlgoritamBaza = new Triangulation(_pOblastCrtanja, _duzinaPauze, _naivni,
                                            _imeDatoteke, _brojSlucajnihObjekata, _poluprecnik);
        break;
    case TipAlgoritma::RECTANGLE_PACKING:
        _pAlgoritamBaza = new rectanglePacking(_pOblastCrtanja, _duzinaPauze, _naivni,
                                               _imeDatoteke, _brojSlucajnihObjekata, _poluprecnik);
        break;
        /*
